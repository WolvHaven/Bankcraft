...
public class SignMysqlInterface implements SignDatabaseInterface {
    ...
    public SignMysqlInterface(PlutusCore plutusCore) {
    ...
    public Location[] getLocations(int type, World world) {
        try {
            ...
            while (result.next()) {
                loadingList.add(new Location(plutusCore.getServer().getWorld(result.getString("world")), Integer.parseInt(result.getString("x")), Integer.parseInt(result.getString("y")), Integer.parseInt(result.getString("z"))));
            }
