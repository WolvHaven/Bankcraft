...
public class SignFlatFileInterface implements SignDatabaseInterface {
    ...
    public int getType(int x, int y, int z, World world) {
        try {
            ...
            plutusCore.getLogger().severe("Could not get Type of Sign in Database!");
        }
        return -1;
    }
    ...
    public boolean createNewSign(int x, int y, int z, World world, int type,
        ...
        try {
            ...
            plutusCore.getLogger().severe("Could not create Sign in Database!");
        }
        return false;
    }
    ...
    public boolean addAmount(int x, int y, int z, World world, String amount) {
        try {
            ...
            plutusCore.getLogger().severe("Could not remove Sign in Database!");
        }
        return false;
    }
    ...
    public String[] getAmounts(int x, int y, int z, World world) {
        try {
            ...
            plutusCore.getLogger().severe("Could not get Amounts of Sign in Database!");
        }
        return null;
    }
    ...
    public Location[] getLocations(int type, World world) {
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (world == null || lineSplit[3].equalsIgnoreCase(world.getName())) {
                    if (type == -1 || Integer.parseInt(lineSplit[4]) == type) {
                        loadingLocationList.add(new Location(plutusCore.getServer().getWorld(lineSplit[3]), Integer.parseInt(lineSplit[0]), Integer.parseInt(lineSplit[1]), Integer.parseInt(lineSplit[2])));
                    }
                }
            }
            ...
            plutusCore.getLogger().severe("Could not get Locations of Signs in Database!");
        }
        return null;
    }
