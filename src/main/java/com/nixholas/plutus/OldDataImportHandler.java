...
public class OldDataImportHandler {
    ...
    public OldDataImportHandler(PlutusCore plutusCore) {
        this.plutusCore = plutusCore;
    }


    public void migratev2_4() {
        String database = plutusCore.getConfigurationHandler().getString("database.typeOfDatabase");
        plutusCore.getLogger().info("Searching for old " + database + " entries...");
        ...
        plutusCore.getLogger().info("Finished migrating of old data! Remember to set updateToUUID to false in the config.yml!!!");
    }
    ...
    private void updateName(String account) {
        Connection conn = ((DatabaseManagerMysql) plutusCore.getDatabaseManagerInterface()).getConnection();
        try {
            ...
            preparedUpdateStatement.setString(1, plutusCore.getUUIDHandler().getUUID(account).toString());
    ...
    private String[] getOldSqlEntries() {
        ...
        try {
            ...
            while (result.next()) {
                try {
                    plutusCore.getUUIDHandler().getUUIDwE(result.getString("player_name")).toString();
    ...
    private void fixFlat() {
        ...
        for (File file : files) {
            try {
                newName = plutusCore.getUUIDHandler().getUUIDwE(file.getName().split("\\.")[0]).toString();
