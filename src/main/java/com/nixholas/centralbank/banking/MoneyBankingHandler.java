...
public class MoneyBankingHandler implements BankingHandler<Double> {
    ...
    public synchronized boolean grantInterests(Player observer) {
        ...
        for (UUID accountName : centralBank.getMoneyDatabaseInterface().getAccounts()) {
            ...
            Player player;
            if ((player = centralBank.getServer().getPlayer(accountName)) != null && (centralBank.getConfigurationHandler().getString("interest.broadcastMoney").equals("true") || CentralBank.perms.has(player, "Centralbank.interest.broadcastmoney"))) {
